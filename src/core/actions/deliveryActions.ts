...
export const fetchDriverDelivery = createAsyncThunk(
...
// Ici on accepte un objet { id, driverId }
export const adminConfirmDelivery = createAsyncThunk(
  "deliveries/confirmAdmin",
  async ({ id, driverId }: { id: string; driverId: string }) => {
    return await confirmAdminDelivery(id, driverId);
  }
);
